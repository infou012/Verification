...
Theorem beq_id_refl : forall x, true = beq_id x x.
  Proof.
    intros [x]. unfold beq_id. apply beq_nat_refl.
Qed.
...
Eval compute in examplemap (Id 8).
...
Proof.
  ...
  intro. case (beq_id x x0); reflexivity.
Qed.
...
Proof.
  intros. unfold t_update. 
  apply functional_extensionality.
  intro. destruct (beq_idP x1 x).
  - rewrite <- e. destruct (beq_idP x2 x1).
    (* goal: False then destruct the contradictory hypothesis *)
    + subst. exfalso. destruct H. reflexivity.
    + reflexivity.
  - reflexivity.
Qed.
    (* ################################################################# *)
...
Proof.
  ...
  inversion Heval. subst. clear Heval. apply t_update_eq.
Qed.  
...
| NW_seq :  forall c1 c2, no_whilesR c1 -> no_whilesR c2
                          -> no_whilesR (c1;;c2)
| NW_ifb : forall b c1 c2, no_whilesR c1 -> no_whilesR c2
...
Proof.
  ...
  - induction c.
    ...
    + constructor.       
    + constructor.
      * inversion H. apply andb_true_iff in H1. apply IHc1.
        destruct H1 as [H1_1 H1_2]. assumption.
      * inversion H. apply andb_true_iff in H1. apply IHc2.
        destruct H1 as [H1_1 H1_2]. assumption.
    + constructor.
      * inversion H. apply andb_true_iff in H1. apply IHc1.
        destruct H1 as [H1_1 H1_2]. assumption.
