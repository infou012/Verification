...
Lemma hoare_sub_ex2:
  {{(fun st => and (0 <= st X) (st X <= 5)) [X |-> (ANum 3)]}}
  (X ::= ANum 3)
...
Choose a = X+1. contradiction X = X+1
*)
...
Proof. 
  ...
  intros.
  split. 
  -inversion H. rewrite t_update_shadow. subst.
   admit.
  -admit.     
Qed.
...
Proof.
  ...
  apply hoare_asgn.
  intros s H.
  unfold assn_sub. simpl. unfold t_update. simpl. apply H.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre. 
    apply hoare_asgn. 
    intros s H.
    unfold assn_sub. simpl. unfold t_update. simpl. tauto.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    simpl. intros  st [_ H]. apply le_plus_minus. apply leb_complete.
    assumption. 
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st \\ st''
  | E_IF1_True : forall (st st': state) (b: bexp) (c: com),
      beval st b = true ->
      c / st \\ st' -> (IF1 b THEN c FI) / st \\ st'
  | E_IF1_False : forall (st st': state) (b: bexp) (c: com),
...
Theorem hoare_if : forall P Q b c,
  ...
  {{P}} (IF1 b THEN c FI) {{Q}}.
Proof.
  intros P Q b c HTrue HFalse st st' HE H.
  inversion HE. subst. 
  - apply (HTrue st st').
    assumption.
    split. assumption.
    easy.
  - apply (HFalse st st'); subst.
    + constructor.
    + split. assumption.
      unfold not, bassn. rewrite H4. easy.
Qed.
